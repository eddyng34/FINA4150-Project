...
def fetch_eth_options_with_prices():
    ...
    price_df = pd.DataFrame(resp_price["result"])[["instrument_name", "mark_price"]]
...
def implied_vol(price, S, K, T, r=0, q=0):
...
def build_iv_surface(df, S0, r=0):
    df["IV"] = df.apply(lambda row: implied_vol(row["mid"], S0, row["K"], row["T"], r), axis=1)
...
# ========================================
def calibrate_lv(iv_surf, S0, r=0, T_max=0.3, N_T=30, N_K=70):
    ...
    return RectBivariateSpline(T_grid, K_grid, lv, kx=3, ky=3, s=0)
